...
.directive('onFinishRender', ['$timeout', '$ionicSlideBoxDelegate' ,function ($timeout, $ionicSlideBoxDelegate) {
    return {
        restrict: 'A',
	...
	function($timeout, $window, $ionicSlideBoxDelegate, $ionicScrollDelegate) {
		...
		return {
			...
			link : function(scope, element, attrs, ngModel) {
				...
				function renderScrollableTabs(){
					...
					if(typeof attrs.tab === 'undefined' || (totalTabs <= initialIndex) || initialIndex < 0){
						initialIndex = Math.floor(icons.length/1);
					}
				...
				function setPosition(index){
					...
					var middle = iconsDiv[0].offsetWidth/1;
					...
					var leftStr = (middle  - (curElLeft) -  curElWidth/1 + 5);
					//If tabs are not scrollable
					if(!scrollDiv){
						var leftStr = (middle  - (curElLeft) -  curElWidth/1 + 5) + "px";
			...
			controller : function($scope, $attrs, $element) {
				...
				$scope.slideHasChanged = function(index, callbackfunc){
